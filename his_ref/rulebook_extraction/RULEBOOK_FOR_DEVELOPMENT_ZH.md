...
实现补全（高优先级校验点）：
- 谈判中仅“可立即改变当前游戏状态”的条目有规则约束；未来承诺类条目均为非约束协议。
- 谈判结束后须按冲动顺序公开声明并由相关各方完整确认；任一方未完整确认则整包协议失效。
- 求和子段在最终回合跳过；求和资格至少满足“曾失本土空间”或“曾有陆将被俘”之一。
- 求和授予 War Winner VP：通常 +1；若战争涉及奥斯曼则再 +1（共 +2）。
- 对苏格兰/威尼斯宣战会触发法兰西/教廷 2 CP 介入窗口；介入成功即激活对应次要势力并建立新战争关系。
- 宣战限制需要拆分为两层校验：
  1. 常时限制（如：同盟中目标、未触发施马尔卡尔登前的新教相关限制、不可对独立关键城宣战等）。
  2. 仅外交阶段限制（如：本阶段刚和谈或刚结盟对象不可再宣战）。
- 行动阶段被事件强制触发的开战，也必须立即更新外交状态；若双方海军同海区，立即海战（平局双方都撤退）。
...
- 路径中间节点（终点除外）还必须无敌方单位/将领（含海军单位）。
- 若 `Schmalkaldic League` 尚未触发，LOC 不得穿过选帝侯空间。
- 经海区追踪 LOC 时，必须由友控港口连接，且相关海区需有友方海军支撑。
...
- 限制移动、建造、计分与部分海盗防御效果。
- 可被对应行动清除。
- 通过 `Control Unfortified Space` 移除动乱时，不要求该目标空间具备 LOC（与翻政治控制不同）。
...
目的地/编队补充约束：
- 进入“他方控制空间”仅在两种关系下合法：`atWar` 或 `allied`。
- 单独陆将可移动，但不得进入敌控空间或含敌军空间；若在无防御空间被敌军进入则立即被俘。
- 本冲动中已输过野战、或正处于围城关系中的单位堆，不能再次用于主动移动。

拦截：
- 邻接且满足条件的敌军可尝试。
- 多方可拦截时按冲动顺序。
- 首个成功拦截会阻断其他拦截者。
- 同一单位堆每冲动最多尝试一次拦截；不得跨山口拦截；被围单位不得拦截。
- 若移动目标是“未被围攻的友方有防御空间”，该次移动不能被拦截。
...
- 只能移动到相邻海区/港口；进入他方控制港口的前提是港内已有敌方海军单位。
- 热那亚/威尼斯海军及 Andrea Doria 不得进入 Atlantic Ocean。
...
- 冲动结束前必须回到港口。
- 若登陆后野战失败，运输部队会被消灭。
- 开始海运时该冲动需至少还可支付 2 CP（保证有能力离海入港）。

海盗：
- 奥斯曼专属，2 CP，每海域每回合最多一次。
- 先由目标方进行反海盗掷骰，再由存活海盗船进行劫掠掷骰。
- 每个劫掠命中由目标方三选一承受代价（失舰、失牌、给奥斯曼海盗 VP）。
- 反海盗骰来源需分别计算：目标方本海区中队（每个 2 骰）+ 相邻海区/港口的交战方中队（每个 1 骰）+ 相邻要塞防御骰（每要塞 1 骰，关键城不提供）。
- 动乱或被围状态要塞不提供反海盗骰。
- 奥斯曼海盗 VP 全局上限 10；目标方手牌为 0 时不可选择“给奥斯曼抽牌”作为代价。
...
- 改革家修正：同空间 +2，邻接空间 +1（新教侧）。
- 耶稣会大学修正：同空间 +2，邻接空间 +1（教廷侧）。
- 单位堆修正：同阵营单位堆在同/邻接空间提供与上同级修正，但同空间不因单位数量叠加多次。
- 英格兰单位宗教阵营随君主变化：
  - Edward VI / Elizabeth I：新教阵营；
  - Mary I：天主教阵营；
  - Henry VIII：无阵营（不参与宗教修正）。
- 奥斯曼单位始终无宗教阵营，不提供也不受宗教修正。
- 辩士使用限制：
  - 单个辩士每回合最多承诺一次，冬季复原；
  - 同一冲动中新教与教廷各最多使用 1 个“改宗/反改宗/翻译类”辩士加值；
  - 提供“额外尝试次数”的辩士必须在该串尝试开始前承诺。
...
回驻与损耗补全：
- 先回海军再回陆军；借入舰队在“移除借舰标记”后按拥有者身份回其最近合法港口。
- 陆军回驻的合法路径要求：除起点外路径空间须友控且无动乱；与 LOC 不同，本步骤忽略敌军单位阻断。
- 普通有防御空间驻军上限 4（首都例外）；超限单位必须继续回首都/本土关键城并重新判定路径损耗。
- 首都若被敌控，原本必须回首都的单位改为直接消灭。
- 新教无首都：始终回最近可容纳的新教控制有防御空间，必要时拆堆。
- 哈布斯堡可回任一首都；若 `Schmalkaldic League` 未触发，冬季回驻不得穿过/进入选帝侯空间。
- 冬季到期强制事件需支持“未打出自动触发且不授予 2 CP”：
  - `Clement VII`（最晚 T2 冬）
  - `Barbary Pirates`（最晚 T3 冬）
  - `Schmalkaldic League`（最晚 T4 冬）
  - `Paul III`（最晚 T4 冬）
  - `Society of Jesus`（最晚 T6 冬）
...
- 激活后视作其同盟主要势力的军事延伸。
- 匈牙利/波希米亚有特殊永久激活路径。
- 苏格兰/威尼斯可由“被宣战后介入”触发激活，并可能同步引入新的主要势力战争关系。
- 实现约束：凡校验“是否可攻击/可拦截/可进入敌军空间”时，需按“活跃次要势力继承其同盟主要势力敌对关系”判定（双向生效，含次要势力对主要势力、次要势力对次要势力）。

独立关键城：
- 可被主要势力夺取并长期维持，除非被特定事件改回独立。
- `Metz`、`Milan`、`Florence`、`Tunis` 属独立关键城目标；可直接围攻，无需先宣战。
...
6. 平手按前回合 VP 逐回溯打破。
- 建议实现为“每次状态变更后可触发增量胜利检查”，尤其是：关键城控制变化、宗教翻转、战争胜者 VP、新大陆结算。
