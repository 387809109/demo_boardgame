...
> **注意**: P0 核心框架及角色已实现并通过 74 项单元测试，但尚未进行手动端到端测试。UI 渲染、多人联机流程仍需实际验证。
...
5. **后端广播优化** - 广播器实现 (T-B100-T-B102)
6. **监控与日志** - 统计信息和健康检查 (T-B110-T-B113)

### 低优先级

7. **狼人杀 P2-P3 角色** - 高级及扩展角色 (T-F078, T-F079)

### 可选功能 (Optional)

8. **桌面应用打包** - Electron 打包为独立可执行文件 (T-F120-T-F122)
   - 用户无需安装编程环境即可运行
