...
#### Here I Stand (HIS) 🔶 开发中

| 任务 | 描述 | 状态 |
|------|------|------|
| 数据提取 | 卡牌(135)、将领(38)、地图数据(134地/15海) | ✅ |
| 地图渲染 | SVG 地图 (形状区分、语言区、海域、新世界面板) | ✅ |
| 地图交互 | 拖拽平移、缩放、空间点击/悬浮提示 | ✅ |
| 游戏状态 | 初始化、6势力、宗教/控制/围城/叛乱状态 | ✅ |
| 阶段系统 | 9阶段 (luther_95→victory_determination) | ✅ |
| 军事行动 | 移动编队、征募、雇佣、骑兵、舰队、海盗、控制、突击 | ✅ |
| 宗教行动 | 发表论文、翻译圣经、辩论、烧书、建造圣彼得、耶稣会 | ✅ |
| 外交行动 | 宣战、求和、谈判、赎回将领 | ✅ |
| 新世界行动 | 探索、殖民、征服 | ✅ |
| 战斗系统 | 野战、围城突击、拦截、撤退 | ✅ |
| 规则审计修复 | isFortified统一、拦截2d6、辩论随机防御方、赎回抽牌 | ✅ |
| UI信息面板 | 势力面板、外交面板、详情面板、宗教面板、手牌面板 | ✅ |
| 地图指示器 | 围城红圈/S标记、叛乱橙色!标记 | ✅ |
| 行动面板 | 阶段感知、CP行动、外交/部署/路德/沃尔姆斯/子交互面板 | ✅ |
| 交互选择系统 | SelectionManager多步选择、空间高亮、区域/势力/部队选择器 | ✅ |
| 事件卡牌 | 卡牌事件处理逻辑 | 🔶 |
| AI 对手 | 单人模式AI策略 | ⬜ |
| 单元测试 | 955 tests passing | ✅ |

> **HIS 当前进度**: 核心游戏逻辑完整 (军事/宗教/外交/新世界)，UI已实现完整的交互选择系统 (多步目标选择、地图高亮、区域/势力/部队选择器)。事件卡牌处理和AI对手待实现。
...
- ✅ HIS 规则一致性审计与修复
  - `isFortified()` 统一：新增 `state-helpers.js:isFortified(sp)` (fortress OR key)，更新 combat-actions/retreat/military-actions/siege-actions/index.js
  - 拦截机制重写：1d6 阈值→2d6+将领评级≥9，奥斯曼骑兵加成，opts 参数兼容
  - 辩论修复：防御方随机选择（非最优）、第二轮平局无结果、燃烧阈值修正
  - 外交修复：赎回改为抽牌（非VP）、教廷-奥斯曼永久联盟禁止
  - 围城修复：突击时攻守双方将领骑点加骰
  - 常量修正：哈布斯堡首都 Brussels→Valladolid
  - 更新全部相关测试 (955 tests passing)

- ✅ HIS UI 交互系统实现
  - 新增 `ui/action-panel.js`：阶段感知行动面板 (CP行动/外交/部署/路德/沃尔姆斯/子交互)
  - 新增 `ui/selection-manager.js`：多步目标选择状态机 (25+行动类型定义)
  - 新增地图指示器：围城红圈+S标记、叛乱橙色!标记 (`map-renderer.js`)
  - 新增空间高亮：选择流程中高亮合法目标空间
  - 新增选择提示栏：蓝色横幅显示当前步骤指令+取消按钮
  - 新增语言区选择器：5色按钮选择德/法/英/意/西语区
  - 新增势力选择器：6势力按钮选择宣战/求和/谈判目标
  - 新增部队选择器：正规军/雇佣兵/骑兵数量微调+将领复选框
  - 重写 `ui.js`：集成 SelectionManager，空间点击路由、选择流程管理
...
- ✅ HIS 规则一致性修复批次（海战/围城/冬季/外交联动）完成并进入待验收
  - 完成海军行动链路校正：海上移动拦截顺序、港口海战进攻方必撤、双方全灭保留 1 单位、热那亚/威尼斯/Andrea Doria 大西洋限制
  - 完成海盗与反海盗校正：骰源拆分、命中结算三选一、VP 上限 10、空手牌时禁抽牌代价、Barbary Pirates 启用前置与奥斯曼海盗船建造地点限制
  - 完成冬季与回合推进补全：海军损失与将领回归的到期处理、日志与状态追踪、Mandatory 到期触发门控补强
  - 完成小国战争传播修正：`canAttack` 覆盖小国↔大国/小国↔小国敌对判定，并用于陆军移动与拦截合法性
  - 自动化验证通过：`npm --prefix frontend run test -- src/games/his`（`952` tests passing）
...
- ✅ 狼人杀 P1 角色：白痴 (Idiot) 完成
  - 新增角色文档：`docs/games/werewolf/roles/idiot.md`
  - 实现首次处决翻牌免死 + 失去投票权（白天投票/队列/UI 联动）
  - 更新狼人杀测试并通过：`frontend/src/games/werewolf/index.test.js`

- ✅ 狼人杀 P1 角色：小丑 (Jester) 完成
  - 新增角色文档：`docs/games/werewolf/roles/jester.md`
  - 实现“被白天处决即单独获胜”胜利判定（含 `winnerPlayerIds` 排名支持）
  - 更新结束面板与角色文案，新增 Jester P1 测试套件
  - 验证通过：`npm test -- src/games/werewolf/index.test.js` (151 tests), `npm test -- src/games/werewolf/ui.test.js` (3 tests)

- ✅ 狼人杀 P1 角色：义警 (Vigilante) 完成
  - 新增角色文档：`docs/games/werewolf/roles/vigilante.md`
  - 实现射杀次数、误杀惩罚（失能/下一夜自杀）与反噬不可被救规则
  - 更新夜间 UI（义警行动面板）与配置项（首夜开枪、误杀惩罚、射击次数）
  - 验证通过：`npm test -- src/games/werewolf/index.test.js` (161 tests)

- ✅ 狼人杀 P1 角色：魔笛手 (Piper) 完成
  - 新增角色文档：`docs/games/werewolf/roles/piper.md`
  - 实现夜间魅惑行动、魅惑状态持久化、夜间结算与胜利判定
  - 更新夜间 UI（魔笛手多目标魅惑面板）与配置项（每晚人数/自魅/重魅/存活胜利/公开名单）
  - 验证通过：`npm test -- src/games/werewolf/index.test.js` (173 tests), `npm test -- src/games/werewolf/ui.test.js` (3 tests)
