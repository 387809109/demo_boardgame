 ...
 * Adapted from api/services/rules-loader.js for MCP server (no logger dependency)
 * @module lib/rules-loader
 */
...
/**
 * Resolve docs/games path — supports both local dev (mcp/lib/../../docs/games)
 * and deployed scenarios.
 */
...
export function loadAllRules() {
  ...
  try {
    ...
    console.error('Failed to read docs/games directory', {
      path: DOCS_BASE,
      error: err.message,
    });
    return;
  ...
  for (const gameId of gameDirs) {
    ...
    console.error(`Loaded rules for "${gameName}"`, {
      gameId,
  ...
  console.error(`Rules loader ready: ${gameRulesIndex.size} game(s) loaded`);
}
...
/**
 * Get all chunks for a game, optionally filtered by type
 * @param {string} gameId
 * @param {string} [category='all'] - 'all' | 'rule' | 'role' | 'mechanism'
 * @returns {RuleChunk[]}
 */
export function getChunksByGame(gameId, category = 'all') {
  if (!gameRulesIndex.has(gameId)) return [];
  const index = gameRulesIndex.get(gameId);
  if (category === 'all') return index.chunks;
  return index.chunks.filter(c => c.type === category);
}
