...
**原则**：在开发新游戏时，AI 玩家逻辑作为**可选的非优先开发事项**，而非必须功能。

**背景**：
- AI 逻辑开发成本高，需要针对每个游戏单独设计策略
- 部分游戏（如社交推理类）难以实现有意义的 AI
- 联机功能优先于单机 AI

**实践要点**：
1. **游戏配置标记**：使用 `supportsAI` 字段标记游戏是否支持 AI
   ```json
   {
     "id": "uno",
     "supportsAI": true,  // 支持 AI
     ...
   }
   {
     "id": "werewolf",
     "supportsAI": false, // 不支持 AI
     ...
   }
   ```
