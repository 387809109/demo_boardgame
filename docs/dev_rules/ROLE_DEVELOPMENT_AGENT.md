...
- **胜利条件优先级**：参考 `RULES.md` 第 9.1 节的胜利条件优先级表。第三方阵营角色**必须**明确指定在优先级表中的位置，并说明与现有胜利条件的冲突处理
...
- [ ] 数据结构清晰定义
- [ ] 夜间行动优先级合理分配
- [ ] 胜利条件优先级明确（第三方阵营必须指定在优先级表中的位置）
- [ ] UI 交互方式明确定义（选择模式、面板内容、徽章）

### ⛔ 人工审核关卡（必须）

**阶段 1 完成后，必须暂停并等待用户审核文档，确认后才能进入阶段 2。**

**操作**：
1. 向用户报告阶段 1 完成，列出生成的文档路径
2. 提供文档摘要（角色类型、阵营、优先级、关键交互）
3. **明确询问用户**：「请审核文档，确认无误后回复"继续"以进入代码实现阶段」
4. **等待用户明确确认**后才继续执行阶段 2

**审核要点**（供用户参考）：
- [ ] 角色能力描述是否准确
- [ ] 夜间行动优先级分配是否合理
- [ ] 胜利条件及其优先级是否明确（第三方阵营须指定在优先级表中的位置）
- [ ] 与其他角色的交互是否正确
- [ ] 边界情况是否覆盖完整
- [ ] 数据结构设计是否合理

**注意**：如果用户提出修改意见，先修改文档直到用户确认，再进入阶段 2。
