...
## 处置（2026-06-15 结案：接受并记录）

**决定**：#Y **接受为可接受的 HIS 固有非对称,结案,不改平衡**。依据:

1. **根因属设计性强势**:France 的领先 100% 来自城堡(Chateaux)VP 引擎——这是 France 的招牌机制(其他势力无等价的被动持续 VP 渠道)。France 经城堡领先是规则层面的设计意图,非引擎/ bot 缺陷。
2. **失衡已显著减弱**:#AA/#AB/#AC + 门控修复后,种子样本 France 降到 **5/8**(此前 ~8/8);England 2、Hapsburg 1 也能赢。引擎结算全程 0 异常。
3. **非阻塞**:不影响引擎正确性与测试推进。

**不做**:不下调 France 城堡优先级,不上调垫底家权重——避免为追求胜率均等而扭曲各势力的设计特性(HIS 本就非对称)。

**留作可选(若日后重启平衡工作)**:上文「后续步骤」2–5(前台跑 30 局上置信区间、单杠杆 A/B、r 扫描)。遥测(`getVpBreakdown` 含 `chateaux` + `_runHisBotBatch`)已就绪,随时可复用。
