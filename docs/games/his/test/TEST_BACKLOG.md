...
- **整卡 no-op（卡几乎只设该字段，最高优先）**：✅ **#6 Leipzig Debate 已修**（2026-06-18）——卡面「召集
  ...
  **被消费**的 `pendingDebate`（旧 `pendingLeipzigDebate` 从此不再写）。原 2 条测试锁定 bug，已改为断言真实开
  辩（攻方=papal、指定辩手生效、被屏蔽者不会成为守方）。**仍待**：#105 Treachery（`pendingTreacheryAssault`，
  应发起背叛突击）、#205 Diplomatic Pressure（`pendingDiplomaticPressure`）、#215 Machiavelli
  （`pendingMachiavelliChoice`）——均需新增解析流程（含交互选择），逐卡核实卡面规则后实现，勿凭空补。
