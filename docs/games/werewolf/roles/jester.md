...
| **优先级** | `P1`（进阶角色） |
| **夜晚行动** | 无 |
| **胜利条件** | 在白天被投票处决（`execution`）— **胜利优先级 1（最高）** |
...
`none` — 小丑无主动技能，能力通过被处决被动触发，无需选择目标。
...
无需暴露特殊状态字段。小丑身份在游戏结束时通过角色公开信息展示。

### 白天 UI 影响

- **游戏结束面板**：当小丑通过处决获胜时，结束面板需显示"小丑胜利"作为获胜阵营
- **角色揭示列表**：游戏结束时角色揭示中小丑显示为对应中文名
